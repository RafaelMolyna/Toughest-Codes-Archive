...
select 
	x.departamento as "Nome Departamento",
	count(x.matr) as "Numero de Empregados",
	round(avg(x.salario), 2) as "Media Salarial",
	round(max(x.salario), 2) as "Maior Salario",
	(case
		when min(x.salario) = 0 then 0
		else round(min(x.salario), 2)
...
group by x.departamento
order by avg(x.salario) desc;
