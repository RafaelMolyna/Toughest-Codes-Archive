...
from
	empregado e
	inner join divisao div on div.cod_divisao = e.lotacao_div
	inner join departamento dep on dep.cod_dep = e.lotacao
	join (
		select
			e.matr,
		...
		from
			...
			join vencimento v on ev.cod_venc = v.cod_venc on e.matr = ev.matr
		group by
			e.matr
	...
	join (
		...
		from
			...
			join desconto d on ed.cod_desc = d.cod_desc on e.matr = ed.matr
		group by
			e.matr
